...
class Peliculas_popular extends Component {
  ...
  render() {
    return (
      <section className="container_peliculas">
        {this.state.array_pelicula.map((pelicula) => (<Pelicula_individual key={pelicula.id} imagen={`https://image.tmdb.org/t/p/w500${pelicula.poster_path}`} nombre={pelicula.title} descripcion={pelicula.overview}/>))}
      </section>
    );
  }
}
