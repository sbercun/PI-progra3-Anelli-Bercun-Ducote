...
function Peliculas_popular(props) {
    ...
    return (
      <section className="pelicula-popular">
        <div className="container_peliculas">
          {peliculas.slice(0,6).map((pelicula) => (<Item_individual key={pelicula.id} id={pelicula.id} imagen={`https://image.tmdb.org/t/p/w500${pelicula.poster_path}`} nombre={pelicula.title} descripcion={pelicula.overview} tipo="movie"/>))}
        </div>
